...
from verl.trainer.ppo.metric_utils import (
    ...
    compute_data_metrics_before_dyn_sampling
)
...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path):
        ...
        with open(filename, "w") as f:
            for i in range(n):
                entry = {k: self._convert_numpy_types(v[i]) for k, v in base_data.items()}
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # update 8: report clip ratio
            # Calculate clip ratio for each response
            response_lengths = test_output_gen_batch.batch["attention_mask"][:, -output_ids.shape[1]:].sum(-1)
            max_response_length = output_ids.shape[1]
            # Create list indicating whether each response was clipped (1) or not (0)
            clipped = (response_lengths == max_response_length).int().tolist()
            reward_extra_infos_dict["clipped"].extend(clipped)
        ...
        for data_source, var2metric2val in data_src2var2metric2val.items():
            ...
            for var_name, metric2val in var2metric2val.items():
                if var_name == "clipped":
                    pfx=f"val-core/{data_source}/{var_name}/clip_ratio"
                    metric_dict[pfx] = metric2val['clip_ratio']
                    continue
    ...
    def fit(self):
        ...
        #======dynamic sampling update start=======
        # update 1: next 4 lines to initialze parameters
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # update 2: substitut batch -> new_batch 
                ...
                with _timer("step", timing_raw):
                    ...
                    # upadate 3: deal with reward computing: move reward [from (after) global_valid_tokens] here
                    with _timer("reward", timing_raw):
                        ...
                        # update 4: move token_level socres [from "adv" calclulation] here
                    ...
                    # update 5: deal with filter_groups_enable
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        for idx, traj_from_prompt_uid in enumerate(new_batch.non_tensor_batch["uid"]):
                            if traj_from_prompt_uid in kept_prompt_uids:
                                ...
                                drop_traj_idxs.append(idx)

                        drop_batch = new_batch[drop_traj_idxs]
                        new_batch = new_batch[kept_traj_idxs]
                        aux_batch = new_batch if aux_batch is None else DataProto.concat([aux_batch, new_batch])

                        aux_drop_batch = drop_batch if aux_drop_batch is None else DataProto.concat([aux_drop_batch, drop_batch])
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            batch = aux_batch[:traj_bsz]
                            drop_batch = aux_batch[traj_bsz:]

                            if len(drop_batch) > 0:
                                print(f"Kept {len(batch)} samples, {len(drop_batch)} remaining.")
                                aux_drop_batch = drop_batch if aux_drop_batch is None else DataProto.concat([aux_drop_batch, drop_batch])
                            else:
                            ...
                            #aux_batch = aux_batch[traj_bsz:]
                            #if len(aux_batch) > 0:
                            #    print(f"Kept {len(batch)} samples, {len(aux_batch)} remaining.")
                            #else:
                            #    print("No remaining samples.")


                    #=====dynamic sample updating end======#
                    # update 7: monitor batches before dynamic sampling
                    if self.config.algorithm.filter_groups.enable:
                        aux_drop_batch = batch if aux_drop_batch is None else DataProto.concat([aux_drop_batch, batch])
                        pre_metrics = compute_data_metrics_before_dyn_sampling(batch=aux_drop_batch, use_critic=self.use_critic)
                    ...
                    if rollout_data_dir:
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            self._dump_generations(
                                ...
                                outputs=outputs,
                                scores=scores,
                                reward_extra_infos_dict=batch.non_tensor_batch,
                                dump_path=rollout_data_dir,
                            )

                            if self.config.algorithm.filter_groups.enable:
                                drop_data_dir = os.path.dirname(rollout_data_dir)
                                drop_data_dir = os.path.join(drop_data_dir, "drop_generations")
                                inputs = self.tokenizer.batch_decode(aux_drop_batch.batch["prompts"], skip_special_tokens=True)
                                outputs = self.tokenizer.batch_decode(aux_drop_batch.batch["responses"], skip_special_tokens=True)
                                scores = aux_drop_batch.batch["token_level_scores"].sum(-1).cpu().tolist()
                                self._dump_generations(
                                    inputs=inputs,
                                    outputs=outputs,
                                    scores=scores,
                                    reward_extra_infos_dict=aux_drop_batch.non_tensor_batch,
                                    dump_path=drop_data_dir,
                                )
                ...
                # update 6: deal with num_prompt_in_batch
                timing_raw = defaultdict(float)  # clear timing

                metrics["train/num_gen_batches"] = num_gen_batches
                #batch = None
                #num_prompt_in_batch = 0
                #num_prompt_in_batch = int(len(aux_batch) / self.config.actor_rollout_ref.rollout.n)
                num_prompt_in_batch = 0
                num_gen_batches = 0
                aux_drop_batch = None
