...
def compute_data_metrics_before_dyn_sampling(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    """
    Computes various metrics from a batch of data for PPO training.
    ...
    metrics = {
        # score
        "critic-pre/score/mean": torch.mean(sequence_score).detach().item(),
        "critic-pre/score/max": torch.max(sequence_score).detach().item(),
        "critic-pre/score/min": torch.min(sequence_score).detach().item(),
        # reward
        "critic-pre/rewards/mean": torch.mean(sequence_reward).detach().item(),
        "critic-pre/rewards/max": torch.max(sequence_reward).detach().item(),
        "critic-pre/rewards/min": torch.min(sequence_reward).detach().item(),
        # response length
        "response_length/mean-pre": torch.mean(response_length).detach().item(),
        "response_length/max-pre": torch.max(response_length).detach().item(),
        "response_length/min-pre": torch.min(response_length).detach().item(),
        "response_length/clip_ratio-pre": torch.mean(torch.eq(response_length, max_response_length).float()).detach().item(),
        # prompt length
        "prompt_length/mean-pre": torch.mean(prompt_length).detach().item(),
        "prompt_length/max-pre": torch.max(prompt_length).detach().item(),
        "prompt_length/min-pre": torch.min(prompt_length).detach().item(),
        "prompt_length/clip_ratio-pre": torch.mean(torch.eq(prompt_length, max_prompt_length).float()).detach().item(),
    }
    return metrics
    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                # update 9: deal with clip_ratio of validation
                if var_name == "clipped":
                    metric = {
                        "clip_ratio": np.mean(var_vals),
                    }
